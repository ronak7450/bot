...
<script>
    ...
    function fetchData() {
        ...
        fetch("api.php?mobile=" + mobile)
            .then(res => res.json())
            .then(data => {
                // Check if data is not found (Empty results or success: false in nested data)
                const isNotFound = (data.data && data.data.success === false) || 
                ...
                if (isNotFound) {
                    ...
                    // Extract results array and format for display
                    ...
                    results.forEach((item, index) => {
                        ...
                        displayStr += `FATHER NAME   : ${item.father_name || 'N/A'}\n`;
                        displayStr += `MOBILE        : ${item.mobile || 'N/A'}\n`;
                        displayStr += `ALT MOBILE    : ${item.alt_mobile || 'N/A'}\n`;
                        displayStr += `ADDRESS       : ${item.address || 'N/A'}\n`;
                        displayStr += `CIRCLE        : ${item.circle || 'N/A'}\n`;
                        displayStr += `AADHAAR NO.   : ${item.aadhaar_number || 'N/A'}\n`;
            ...
            .catch(err => {
                panel.classList.add('error-panel');
                typeWriter("[CONNECTION ERROR] FAILED TO REACH SERVER OR SERVICE IS DOWN.", true);
            });
    }
